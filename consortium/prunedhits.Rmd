...
rsid = unique(trimws(strsplit(paste0(db$SNPS, collapse=';'), ';')[[1]]))
rsid = unique(trimws(strsplit(paste0(db$SNPS, collapse=';'), ';')[[1]]))
good = rsid[grepl("^rs*", rsid)]
bad = rsid[!grepl("^rs*", rsid)]
rsid=good
...
```{r}
# Load reference hits
db = read.delim("~/Downloads/gwas-association-downloaded_2017-02-28-cd.tsv", sep = '\t', head = T,stringsAsFactors = FALSE)
...
db = read.delim("~/Downloads/gwas-association-downloaded_2017-02-28-ulcerative.tsv", sep = '\t', head = T,stringsAsFactors = FALSE)
...
db = read.delim("~/Downloads/gwas-association-downloaded_2017-02-28-bmi.tsv", sep = '\t', head = T,stringsAsFactors = FALSE)
...
```{r}
# Load reference hits
db = read.delim("~/Downloads/gwas-association-downloaded_2017-02-28-femoral.tsv", sep = '\t', head = T,stringsAsFactors = FALSE)
...
```{r}
# Load reference hits
db = read.delim("~/Downloads/gwas-association-downloaded_2017-02-28-schizophrenia.tsv", sep = '\t', head = T,stringsAsFactors = FALSE)
...
# Load data
df = read.table('../Data/significant/sczsignificantin.txt', stringsAsFactors = FALSE)
dat = rs_to_pos(rownames(df))
# Find overlap between known hits and data by 1MB ranges
result = overlap_by_million(dat, ref)
...
write.table(hits_pruned,"prunedhitsscz.txt")
```
